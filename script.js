// Script for Ultimate Styling Trustpilot clone
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const REVIEWS = [
        {
            ...
            text: '5 star service as it should be.\nQuick delivery.\nOrdered wrong rear lights, when contacting Pete was helpful advising our car had the face lift and the correct lights to order, return was simple and correct lights arrived next day.\nSimple plug and play lights took less then 20 mins to change both.'
        },
        {
        ...
        },
        {
            ...
            text: 'Excellent customer service. I Ordered a wing mirror glass on a Wednesday night and it was despatched the next day. It was sent 48 hrs and after it still hadn\'t arrived by the following Tuesday it was returned to sender by EVRI. Pete handled the replacement quickly and even refunded the postage. Top marks.'
        },
        {
            ...
            text: 'Easy-to-use website. Excellent communication. Good prices. Excellent quality. Easy to fit.\nThe website was very easy to use to find parts specific to my car. I ordered wing mirror glasses for both sides and they fitted perfectly first time. Would definitely recommend.'
        },
        {
            ...
            title: 'Awful firm',
            text: 'Awful firm. I ordered an item from them via Amazon. The courier they use delivered it to a completely incorrect address, and I have photographic evidence of this. They are failing to resolve this matter despite multiple emails and Amazon\'s involvement. Avoid.'
        },
        {
            ...
            text: 'I order with them the item was very good price and quality.\nChange the delivery company that you give to client to chose ( EVRI).\nI had bad experience with them.\nI order an Ford oil pan/sump and EVRI lost it in transit. Ultimate Styling did send a replacement but it took an extra week.'
        },
        {
            ...
            text: 'Thank you Ultimate Styling. Super fast delivery! Ordered on Friday early afternoon and arrived on a Saturday! Well worth the paying the little extra for premium delivery as it helped me get out of a bind before a long drive on Monday morning. Part fitted perfectly.'
        },
        {
            ...
            text: 'I hardly ever leave reviews, but I needed to let others know about Ultimate Styling. I had been searching EVERYWHERE for a rear windscreen wiper for my Fiat 500X, but the ones you get from well known stores just didn\'t fit. Ultimate Styling had the exact one I needed, ordered Tuesday, arrived Wednesday morning. Couldn\'t be happier.'
        }
    ];
    ...
    if (allReviewsList && typeof REVIEWS !== 'undefined') {
                        ...
                        <div class="full-review-reply">
                            <div class="full-review-reply-content">
                                <div class="full-review-reply-header">
                                    ...
                                    <div>
                                        <div class="full-review-reply-name">Reply from Ultimate Styling</div>
        ...
        // Render in pages so the browser doesn't choke on rendering ~5800 cards at once
        let renderedCount = 0;
        const renderNextPage = () => {
            const slice = expanded.slice(renderedCount, renderedCount + PAGE_SIZE);
            if (!slice.length) return;
            const frag = document.createElement('div');
            frag.innerHTML = slice.map(renderFullReviewCard).join('');
            while (frag.firstChild) allReviewsList.appendChild(frag.firstChild);
            renderedCount += slice.length;
            updateReviewCounter();
        };

        // Replace pagination with a "Load more" button + counter
        const pagination = document.querySelector('.reviews-pagination');
        if (pagination) {
            pagination.innerHTML = `
                <div class="reviews-counter">Showing <span class="reviews-shown">0</span> of ${TOTAL_REVIEWS.toLocaleString()} reviews</div>
                <button class="pagination-btn load-more-btn" type="button">Load more reviews</button>
            `;
        }

        const shownEl = document.querySelector('.reviews-shown');
        const loadMoreBtn = document.querySelector('.load-more-btn');
        function updateReviewCounter() {
            if (shownEl) shownEl.textContent = renderedCount.toLocaleString();
            if (loadMoreBtn) loadMoreBtn.hidden = renderedCount >= expanded.length;
        }
